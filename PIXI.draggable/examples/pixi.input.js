 ...
 //define(["pixi"], function(PIXI) {
    ...
    (function(undefined) {
        ...
        function getFontHeight( obj )
        {
            ...
            {
                return obj.determineFontProperties(obj.style.font).fontSize + obj.style.strokeThickness;
            }
        };

        // rounded rect .. maybe replace this with a pixi graphic?
        function roundedRect(ctx, x, y, w, h, r, bw)
        {
        ...
        PIXI.ButtonFactory = function( can, ctx, opts )
        {
            ...
            if (opts.borderWidth > 0)
            {
                ...
                roundedRect(ctx, shadowLeft, shadowTop, width - shadowWidth, height - shadowHeight, borderRadius, opts.borderWidth);
                ctx.fill();
            ...
            if(!opts.bgImage || !opts.backgroundImage || !opts.bgImage.baseTexture.hasLoaded)
            {
                ...
                roundedRect(ctx, textboxLeft, textboxTop, textboxWidth, textboxHeight, borderRadius, opts.borderWidth);
                ctx.fill();
            }
            else
            {
            ...
            renderTexture: function()
            {
                ...
                if(this.data.ninepatch && PIXI.NinePatch !== undefined)
                {
                    if(this._cachedNinePatch !== this.data.ninepatch)
                    {
                        if(!this._oldTexture)
                        {
                            this._oldTexture = this.texture;
                        }
                        this._cachedNinePatch = this.data.ninepatch;
                        this.ninepatch = new PIXI.NinePatch(this.data.width, this.data.height, this.data.ninepatch, this.data.useFrames);
                        if(this.ninepatch.loaded < 9)
                        {
                            this.ninepatch.onReady(function() {
                                ...
                                this.data.textboxWidth = this.ninepatch.children[4].width;
                                this.data.dirty = true;
                            }.bind(this));
                        }
                        else
                ...
                {
                    this.text.y = (this.data.textboxTop + 2 + ((this.data.textboxHeight - this.text.height)/2)) | 0;
                }
                else if(this.data.valign === "top")
                {
                    this.text.y = this.data.textboxTop | 0;
                ...
                if(this.data.align === "center")
                {
                    this.text.x = (this.data.width - this.text.width) / 2 | 0;
                }
                else if(this.data.align === "right")
                {
                ...
                {
                    this.text.x = (this.data.textboxLeft + this.data.padding);
                }
